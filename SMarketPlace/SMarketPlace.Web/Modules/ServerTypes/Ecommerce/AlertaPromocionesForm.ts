...
export interface AlertaPromocionesForm {
...
export class AlertaPromocionesForm extends PrefixedContext {
    static formKey = 'Ecommerce.AlertaPromociones';
    private static init: boolean;

    constructor(prefix: string) {
        super(prefix);

        if (!AlertaPromocionesForm.init)  {
            AlertaPromocionesForm.init = true;

            var w0 = StringEditor;

            initFormType(AlertaPromocionesForm, [
                'IdPromocion', w0,
                'MapCod', w0,
                'NroLocal', w0,
                'Mecanica', w0,
